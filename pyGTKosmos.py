...
#                        dst = "0030" + dst_entry.get_text()
    ...
    def contacts_on_clicked(self , widget):
        return 1

        
    ## functions read() & write() from http://www.codelibary.com/Python/Address%20Book.html
    def read(self):
        # reading in the diction using argv[1] as the name of the file
        #if len(sys.argv)<2:
        #    print """ Sorry you must give the name of the file on the command line"""
        #    sys.exit(2)
        
        try:
            contact_file = open(database,"r")
        except IOError:
            print "No data file", database, "will be created at the end of the program"
            return 
        contact_list={}
        line = contact_file.readline()
        while line:
            nickname = line[:-1]
            datastring = contact_file.readline()
            if not datastring:
                print "Badly formated data file, extra lines"
            else:
                data = eval(datastring)
                if type(data) != type([]) or len(data)!= 4:
                    print "Badly formated data line:", datastring
                else:
                    contact_list[nickname]=data
            line = contact_file.readline()
    ...
    def write(contact_list):
        contact_file = open(database,"w")
        for nickname in contact_list.keys():
            contact_file.write(str(nickname)+"\n")
            contact_file.write(repr(contact_list[nickname])+"\n")
        contact_file.close()
    ## end of functions read() & write() from http://www.codelibary.com/Python/Address%20Book.html          
    ...
    def __init__(self):
            ...
            window.set_size_request(400, 300) # bigger
            ...
            database = directory + '/contacts.db'  
            ...
            # contacts button
            contacts_button = gtk.Button("Add Contact")
            contacts_button.connect("clicked", self.options, window)
            hbox.pack_start(contacts_button, False, False, 2)                         
            # end of contacts button

            dst_entry = gtk.Entry(max=129) # 10 numbers * 10 free sms + 9 seperators
